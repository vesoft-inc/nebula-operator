...
func (h *ValidatingAdmission) Handle(_ context.Context, req admission.Request) (resp admission.Response) {
	...
	if operation == admissionv1.Create {
		if allErrs := validateNebulaAutoscalerCreate(obj); len(allErrs) > 0 {
			return admission.Errored(http.StatusUnprocessableEntity, allErrs.ToAggregate())
		}
	} else if operation == admissionv1.Update {
		oldObj := &v1alpha1.NebulaAutoscaler{}

		if err := h.Decoder.DecodeRaw(req.AdmissionRequest.OldObject, oldObj); err != nil {
			return admission.Errored(http.StatusBadRequest, err)
		}

		if allErrs := validateNebulaAutoscalerUpdate(obj, oldObj); len(allErrs) > 0 {
			return admission.Errored(http.StatusUnprocessableEntity, allErrs.ToAggregate())
		}
	}
